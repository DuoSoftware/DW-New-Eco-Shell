    (function () {
        var duoworldFrameworkShellLauncherUserprofileCtrl = function ($scope, $rootScope, $mdDialog, $window, $http, $auth, notifications, $charge, $objectstore, $storage, $uploader, $ocLazyLoad, shellDataFactory, $stateParams) {


            //variable declaration
            var profile = this;
            profile.shellConfig = shellDataFactory.getShellConfig();
            profile.account = {};
            profile.ledger = [];
            ...
            profile.account.AccountCards = shellDataFactory.getAccountCards();
        ...
        , {
            ...
            console.log($stateParams.profileIndex);
            if ($stateParams.profileIndex == undefined | $stateParams.profileIndex == "") {
                console.log("no index passed");
                profile.profileIndex = 0;
            } else {
                profile.profileIndex = $stateParams.profileIndex;
            }


            shellDataFactory.setAccountCards(profile.account.AccountCards);
            ...
            function showCheckout(plan, ev) {
                $mdDialog.show({
                    controller: 'duoworld-framework-shell-checkout-ctrl as co',
                    templateUrl: 'partials/modal-templates/partials.modal-templates.checkout.html',
                    parent: angular.element(document.body),
                    targetEvent: ev,
                    clickOutsideToClose: true,
                    locals: {
            ...
            function upgradePlan(plan, ev) {
                console.log(plan);
                if (profile.account.AccountCards.length == 0) {
                    newCard(ev);
                } else {
                    showCheckout(plan, ev);
                };
            }

        };
        duoworldFrameworkShellLauncherUserprofileCtrl.$inject = ['$scope', '$rootScope', '$mdDialog', '$window', '$http', '$auth', 'notifications', '$charge', '$objectstore', '$storage', '$uploader', '$ocLazyLoad', 'shellDataFactory', '$stateParams'];
