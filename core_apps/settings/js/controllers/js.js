angular.module('userAdministration').controller('AppCtrl', function ($scope, $rootScope, $mdDialog, $location, $state, $timeout, $q, $http, notifications, $auth) {
        ...
        $scope.changeTab = function (ind) {
            ...
            switch (ind) {
                case 0:
                    $location.url("/launcher/userAdmin/users");

                    break;
                case 1:
                    $location.url("/launcher/userAdmin/groups");

                    break;
                case 2:
                    $location.url("/launcher/userAdmin/shareApps");
        ...
        function loadOwernTenantDetails() {
            $http.get("/auth/GetSession/" + $auth.getSecurityToken() + "/" + window.location.hostname)
                .success(function (data) {
                    ...
                    $scope.owernUser = {
                        DataCaps: "",
                        Domain: "eranga.dev.duoworld.com",
                        Email: "eranga@duosoftware.com",
                        Name: "Eranga Kariyawasam",
                        SecurityToken: "ba599a4f06f6db1393a69f0e6c3cd2fa",
                        UserID: "7b8f2231089ac71de8af466ff903034e",
                        Username: "eranga@duosoftware.com",
                    }
                });
        }
...
.controller('usersCtrl', function ($scope, $mdDialog, $location, $state, $timeout, $q, $http, $auth, $v6urls) {
    ...
    function getuserdatabulk() {
        ...
        $http({
            ...
            $scope.users = [{
                ...
                Company: "",
                Country: "",
                Email: "dfgdfa@duosoftware.com",
                Name: "ygdfgd Kariyawasam",
                PhoneNumber: "",
                ZipCode: "",
            }]
        });
...
.controller('groupsCtrl', function ($scope, $mdDialog, $location, $state, $timeout, $apps, $q, $http) {
    ...
    $scope.getallgroup = [{
        ...
        groupname: "dsdsg",
        parentId: "",
        users: ["eranga@duosoftware.com", "devare@divismail.ru"]
        }, {
        groupId: "-959",
        groupname: "asgjfgdsg",
        parentId: "",
    ...
    // .............................
    $scope.groupIndex = 0;
    $scope.showSelectedGroup = function (group, ev) {
    ...
    $scope.createGroup = function (ev) {
        $mdDialog.show({
                ...
                templateUrl: 'core_apps/settings/partials/createGroup.html',
    ...
    $scope.addNewUsersToGroup = function (getusers, ev) {
        ...
        $mdDialog.show({
                ...
                templateUrl: 'core_apps/settings/partials/addNewUsersToGroup.html',
                parent: angular.element(document.body),
                targetEvent: ev,
                locals: {
                    getusers: getusers
...
.controller('shareAppsCtrl', function ($scope, $mdDialog, $location, $state, $timeout, $apps, $q, $http) {
    $scope.showGlobalProgress = true;
    $scope.shareIndex = 0;
    $scope.app = [{
        AppType: "HTML5SDK",
        AppUri: "//",
        ApplicationID: "APP_SHELL_SETTINGS",
        Description: "Settings Application",
        ImageId: "",
        Name: "User Administrator",
        SecretKey: "4aa7ccd3a0e37a5264ac0383cf9366f6",
        iconUrl: "/devportal/appicons/APP_SHELL_SETTINGS.png",
    }]
    ...
    //})
    //
    //.controller('shareCtrl', function ($scope, $http, $state, $auth, $objectstore, $v6urls, $apps, $mdDialog, $stateParams, $mdToast, notifications) {
...
function DialogCreateGroupController($scope, $http, $mdDialog, $objectstore, $auth, $v6urls, $mdToast, notifications) {
...
function DialogaddNewUsersToGroupController($scope, $http, $mdDialog, $objectstore, $v6urls, getusers, $mdToast, notifications) {
    ...
    function createAllUserAndGroup(data) {
        ...
        if ($scope.allContacts.length === 0) {
            $scope.allContacts = [{
                "_lowername": "administrator",
                "email": "admin@duoweb.info",
                "image": "img/user.png",
                "name": "Administrator"
            }];
        }
...
angular.module('userAdministration').directive('firstLetter', function ($compile) {
    return {
        restrict: 'EA',
        transclude: true,
        scope: {
        ...
        template: '<div style="background-color:{{color}};width:inherit;min-height:inherit;background-image:\ url({{icon}});\
    background-repeat: no-repeat;\
    background-size: contain;\
    background-position: bottom center;\
    background-blend-mode: soft-light;" layout="column" layout-align="center center">\
<p style="font-size: 70px;\
    font-weight: 300;\
    color: rgba(255,255,255,0.87);\
    text-shadow: 0px 0px 15px rgba(0,0,0,0.54);">{{name | limitTo :1 | uppercase}}</p></div>',
        link: function (scope) {
            scope.icon = scope.icon;
            var first = scope.name.charAt(0);
