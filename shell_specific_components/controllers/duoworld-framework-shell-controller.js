...
(function () {
    'use strict';
    var duoWorldFrameworkShellCtrl = function ($rootScope, $scope, $state, $http, $location, $mdSidenav, $mdDialog, $mdToast, $presence, $auth, $apps, $v6urls, $helpers, $objectstore, $timeout, $fws, $document, $q, $storage, $mdBottomSheet, $ocLazyLoad, shellDataFactory, $mdMedia) {
        ...
        vm.checkSubscription = checkSubscription;
        ...
        shellDataFactory.getUserProfileDetails()
            .then(function (data) {
                ...
                $rootScope.profilePicture = "images/usercover4.jpg";
        ...
        function loadTenantConfig() {
            ...
            success(function (data, status, headers, config) {
                //return data;
                vm.shellConfig = data.shellConfiguration;
                shellDataFactory.setShellConfig(vm.shellConfig);
                console.log(data);
        ...
        function childApplicationClose() {
            ...
            var route = vm.routeHistory[vm.routeHistory.length - 1].route;
            if (route == undefined) {
        ...
        $scope.$on('$stateChangeSuccess', function (ev, to, toParams, from, fromParams) {
            ...
            if (from.name) {
                var route = vm.routeHistory[vm.routeHistory.length - 1].route;
                if (to.name == route.from && from.name == route.to) {
                    vm.routeHistory.pop();
                } else {
                    vm.routeHistory.push({
                        route: {
                            from: from.name,
                            to: to.name
                        },
        ...
        function checkSubscription() {
            ...
            shellDataFactory.setTenantSubscription(vm.tenantSubscription);
            if (vm.tenantSubscription.expired) {
                $mdDialog.show({
                    controller: checkSubscriptionController,
                    templateUrl: 'partials/modal-templates/partials.modal-templates.subscription-expired.html',
        ...
        function checkSubscriptionController($scope, $mdDialog) {
    ...
    duoWorldFrameworkShellCtrl.$inject = ['$rootScope', '$scope', '$state', '$http', '$location', '$mdSidenav', '$mdDialog', '$mdToast', '$presence', '$auth', '$apps', '$v6urls', '$helpers', '$objectstore', '$timeout', '$fws', '$document', '$q', '$storage', '$mdBottomSheet', '$ocLazyLoad', 'shellDataFactory', '$mdMedia'];
